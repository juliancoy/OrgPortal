...
test('tenant navigation uses its root while the main portal uses the build base', () => {
  ...
  expect(normalizePostLoginPath('/timebanking')).toBe('/')
  expect(normalizePostLoginPath('/timebanking?tab=imports')).toBe('/?tab=imports')
  expect(new URL(portalAuthCallbackUrl('/')).searchParams.get('next')).toBe('/')
