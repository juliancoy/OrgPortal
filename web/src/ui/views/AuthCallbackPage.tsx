...
export function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    const requestedNext = normalizePostLoginPath(queryParams.get('next') || defaultPostLoginPath())
    ...
    if (communityId) {
      ...
      void fetch(`/api/org/api/timebank/communities/${encodeURIComponent(communityId)}`, { signal: controller.signal })
        .then(async (response) => {
          ...
          if (!controller.signal.aborted) window.location.replace(`https://${community.hostname}/p${requestedNext}`)
