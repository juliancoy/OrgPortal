...
app.get("/api/timebank", async (c) => {
  ...
  c.header("Cache-Control", "no-store");
  return c.json({ ...await timebankDashboard(c.env.DB, user ? { id: user.id, name: userName(user) } : null, community.id, c.req.query("request_sort") || "most"), community, can_manage_community: user ? adminUser(user, c.env) : false });
});
