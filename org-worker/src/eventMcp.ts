...
export async function runEventOperation(env: Env, identity: { userId: string; scopes: string[] },
  operation: "list" | "get" | "plan", input: unknown) {
  const args = operation === "list" ? listSchema.parse(input)
  ...
  return executeEventPlan(provider, config, args);
}
...
export async function handleEventMcp(request: Request, env: Env) {
  try {
    ...
    const result = async (operation: "list" | "get" | "plan", args: unknown) => {
      try {
    ...
    server.registerTool("apply_event_changes", { description: "Update an event or grant collaborator access after showing a preview and obtaining user approval. Requires confirm=true. Changes may notify guests and are not atomic; inspect failures before retrying.",
