...
export async function renderEventPoster(event: PosterEvent, publicUrl: string, format: PosterFormat, brand: PosterBrand) {
  ...
  const parts: string[] = [];
  function block(value: string, x: number, y: number, maxWidth: number, size: number, maxLines: number, color = '#172033', weight = 400) {
  ...
  parts.push(`<rect width="${width}" height="${height}" fill="#ffffff"/><rect width="${width}" height="${small ? 8 : 12}" fill="#087f8c"/>`);
  ...
  block(clean(brand.name), brandX, brandY + (small ? 17 : 26), brandWidth, small ? 13 : 22, 1, '#075e68', 700);
  if (brand.tagline) block(clean(brand.tagline), brandX, brandY + (small ? 35 : 52), brandWidth, small ? 10 : 15, 1, '#435362');
  ...
  const titleBottom = block(title, margin, titleY, textArea, titleSize, Math.floor(titleHeight / (titleSize * 1.25)), '#172033', 800);
  ...
  y = block(date, margin, y, textArea, detailSize, 2, '#075e68', 700);
  if (time) y = block(time, margin, y + 3, textArea, detailSize, 1, '#075e68', 700);
  y = block(clean(event.location) || 'Location to be announced', margin, y + (small ? 14 : 22), textArea, small ? 14 : 24, small ? 3 : 2, '#172033', 500);
  ...
  if (descLines > 0) block(clean(event.social_description || event.description), margin, y, textArea, descriptionSize, descLines, '#435362');
  const qrSize = small ? 100 : social ? 168 : 168;
  const qrX = width - margin - qrSize;
  const qrY = social ? 380 : footerTop + (small ? 17 : 28);
  const qr = await QRCode.toString(publicUrl, { type: 'svg', margin: 4, errorCorrectionLevel: 'M', color: { dark: '#101820', light: '#ffffff' } });
  parts.push(`<path d="M${margin} ${footerTop}H${social ? 876 : width - margin}" stroke="#c8d9dc" stroke-width="2"/>`);
  parts.push(qr.replace('<svg ', `<svg x="${qrX}" y="${qrY}" width="${qrSize}" height="${qrSize}" `));
  parts.push(`<text x="${qrX + qrSize / 2}" y="${qrY + qrSize + (small ? 15 : 24)}" text-anchor="middle" fill="#075e68" font-size="${small ? 11 : 16}" font-weight="700">Scan to RSVP</text>`);
  block('Join us', margin, footerTop + (small ? 40 : social ? 33 : 55), social ? 800 : qrX - margin - 16, small ? 23 : social ? 26 : 34, 1, '#075e68', 700);
  block(new URL(publicUrl).hostname, margin, footerTop + (small ? 65 : social ? 64 : 89), social ? 800 : qrX - margin - 16, small ? 12 : 19, social ? 1 : 2, '#435362');
