...
- `hostname`: custom domain used to resolve the tenant.
...
- The D1 `portal_tenants` row and any organization seed rows referenced by `home_org_slug`.
...
2. Add the Cloudflare custom domain and route it to the OrgPortal site/worker.
3. Add PIdP origins/callbacks for the tenant domain.
4. Insert a `portal_tenants` row with `hostname`, `public_base_url`, root `canonical_path_prefix`, branding assets, features, and home settings.
5. Configure MCP resource metadata and provider bindings for enabled integrations.
6. Verify `/`, `/users/login`, `/people`, `/orgs/:slug`, `/events`, `/org-events`, favicon, manifest, CSS, JS chunks, and service worker registration on the tenant domain.
7. Verify `https://codecollective.us/p` behavior still works after deploying the same web artifact.
